...
class Neuron:   
    def __init__(self, sid, out_conn, out_weights, fanout, tau, t_ref, V_rest = 0, V_th = 50, spike_propagation_base_time=None):
        self.tau = tau
        self.t_ref = t_ref
        self.sid = sid
        self.V_rest = torch.tensor(V_rest)
    ...
    def receive_spike(self, time, value):
        ...
        if (time - self.last_spike) > self.t_ref:
            V = self.Vp * torch.exp(torch.tensor(-(time-self.last_receive_spike)/self.tau))
            V = V + value
